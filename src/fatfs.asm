...
.check_is_directory:
    ...
    jr .no_icon                                ;
.filesize:
    ld a, ' ' : ld (de), a : inc de            ; one space after the extension
    ld a, '$' : ld (de), a : inc de            ;
    ld bc, DIR_FileSize-11+2                   ;
    add hl, bc                                 ; HL -> size byte 2 (bits 16..23)
    ld a, (hl) : inc hl : or (hl)              ; bytes 2|3 == 0  ->  file <= 64KB
    jr z, .size16                              ; ... <=64KB: $XXXX (HL -> byte 3)
    ld a, (hl) : or a : jr nz, .size_big       ; byte3 != 0 -> >= 16MB -> $FFFF+
    dec hl                                     ; HL -> byte 2
    ld a, (hl) : and #f0 : jr nz, .size_big    ; bits 20..23 set -> >= 1MB -> $FFFF+
    ld a, (hl) : and #0f                       ; 64KB < size < 1MB: top hex digit = bits 16..19
    add a, #90 : daa : adc a, #40 : daa        ;
    ld (de), a : inc de                        ;
    dec hl                                     ; HL -> byte 1
    jr .print2                                 ; then byte1,byte0 -> real size $XXXXX (5 hex)
.size_big:                                     ; >= 1MB (unplayable): ">too big" marker  $FFFF+
    ld a, 'F'                                  ;
    ld (de), a : inc de                        ;
    ld (de), a : inc de                        ;
    ld (de), a : inc de                        ;
    ...
    jr .term                                   ;
.size16:
    .2 dec hl                                  ; HL -> size byte 1
.print2:
    ld b, 2                                    ; print low word ($XXXX)
2:  ld a, (hl)                                 ; hi
    and #f0                                    ; ...
    .4 rra                                     ; ...
    add a, #90                                 ; ...
    ...
    dec hl                                     ;
    djnz 2b                                    ;
.term:                                         ; NULL-terminate BEFORE the icon lookup, which
    xor a                                      ; clobbers DE (disks_get_icon_by_extension does
    ld (de), a                                 ; ld d,h:ld e,l) - otherwise the string is left
.icon:                                         ; unterminated and trailing garbage is shown.
    pop hl                                     ;
    call disks_get_icon_by_extension           ; A = icon
    ld (ix), a                                 ;
    xor a                                      ; set Z flag
    ret                                        ;
.no_icon:                                      ; directory path (DE still = output pointer)
    xor a                                      ;
...
1:  ld de, var_current_file_name               ; copy file name
    ...
    or 1                                       ; ...
    ret                                        ; ...
1:  ld (var_current_file_size+0), bc           ; file size low word
    inc hl                                     ; high word
    ld c, (hl) : inc hl : ld b, (hl)           ; BC = size high word (c=byte2, b=byte3)
    ld a, b                                    ; implausible high bytes (>= 1 MB)?  Some FAT dir
    or a                                       ; entries (long names near a window boundary) read
    jr nz, .clamp_hi                           ; stale/garbage high bytes on small files - clamp
    ld a, c                                    ; them to 0 so the file still loads as <= 64 KB.
    cp #10                                     ; ...
    jr c, .hi_ok                               ; ... byte2 < 0x10 (< 1 MB) -> plausible, keep
.clamp_hi:
    ld bc, 0                                   ; ...
.hi_ok:
    ld (var_current_file_size+2), bc           ; full (or clamped) 32-bit file size
.check_capacity:                               ; reject if file does not fit detected RAM
    ld a, b                                    ; > ~16 MB (byte3 set)?
    or a                                       ; ...
    jr z, 2f                                   ; ...
    or 1 : ret                                 ; ... too big
2:  ld a, (var_current_file_size+1)            ; pages_needed = ceil(size / 16384)
    rlca : rlca : and 3                        ; ... floor = (size >> 14) = (s2 << 2) | (s1 >> 6)
    ld c, a                                    ; ...
    ld a, (var_current_file_size+2)            ; ... s2 << 2 (s2 < 64 for <= 1 MB)
    add a, a : add a, a                        ; ...
    or c                                       ; ...
    ld c, a                                    ; ... C = floor(size / 16384)
    ld a, (var_current_file_size+0)            ; remainder in low 14 bits? (s0 != 0 || (s1 & 0x3F) != 0)
    or a                                       ; ...
    jr nz, 4f                                  ; ...
    ld a, (var_current_file_size+1)            ; ...
    and #3f                                    ; ...
    jr z, 5f                                   ; ... no remainder
4:  inc c                                      ; ... round up
5:  ld a, (var_file_pages_count)               ; capacity (banks) >= pages_needed?
    cp c                                       ; ...
    jr nc, 3f                                  ; ...
    or 1 : ret                                 ; ... does not fit -> error
3:  ld de, -DIR_FileSize+DIR_FstClusHI-3       ; DEBC = cluster_n
    add hl, de                                 ; ...
    ld e, (hl) : inc hl : ld d, (hl)           ; ...
    ld bc, DIR_FstClusLO-DIR_FstClusHI-1       ; ...
    add hl, bc                                 ; ...
    ...
    call fatfs_get_lba                         ; ...
    jp nz, .err_pop2                           ; ...
    ld hl, 0                                   ; HL = file_position (low 16)
    xor a : ld (var_file_pos_hi), a            ; ... bits 16..19 = 0
...
.switch_page_if_required:
    ...
    push af : push bc                          ; ...
    call file_switch_page                      ; ... (no longer reads a byte; HL preserved)
    pop bc : pop af                            ; ...
.read_from_disk:
...
.calc_file_position:
    ...
    add a, h                                   ; ...
    ld h, a                                    ; ...
    jr nc, .check_size                         ; crossed a 64 KB boundary?
    ld a, (var_file_pos_hi)                    ; ... yes: carry into bits 16..19
    inc a                                      ; ...
    ld (var_file_pos_hi), a                    ; ...
.check_size:                                   ; done when file_position (20-bit) >= file_size (32-bit)
    push hl : push de : push bc                ;
    ex de, hl                                  ; DE = pos low 16
    ld hl, (var_current_file_size+0)           ; HL = size low 16
    or a                                       ; ...
    sbc hl, de                                 ; ... size_lo - pos_lo  (CF = borrow)
    ld a, (var_file_pos_hi)                    ; pos byte2 (byte3 = 0)
    ld e, a                                    ; ...
    ld a, (var_current_file_size+2)            ; size byte2 - pos byte2 - borrow
    sbc a, e                                   ; ...
    ld e, a                                    ; ...
    ld a, (var_current_file_size+3)            ; size byte3 - 0 - borrow
    sbc a, 0                                   ; ... CF set -> file_size < file_position
    jr c, 1f                                   ; ...
    or e : or h : or l                         ; ... result == 0 -> file_size == file_position
    jr z, 1f                                   ; ...
    pop bc : pop de : pop hl                   ; file_size > file_position: keep loading
    jr .calc_remain_sectors_in_cluster         ; ...
1:  pop bc : pop de : pop hl                   ; reached end of file
    jp .exit_pop2                              ; ...
